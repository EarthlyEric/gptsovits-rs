    ...
    pub fn extract(&self, audio_16k: &[f32]) -> Result<Array3<f32>> {
        ...
        if let Some(session) = session_guard.as_mut() {
            ...
            if shape_slice.len() == 3 {
                ...
                let ssl_len = input_len / 320;
                Array3::from_shape_vec((1, 768, ssl_len.max(1)), vec![0.0f32; 768 * ssl_len.max(1)])
                    .map_err(|e| anyhow!("Failed to create dummy SSL: {}", e))
            }
        } else {
            // Fallback mock representation for development / testing
            let ssl_len = (audio_16k.len() / 320).max(1);
            Ok(Array3::zeros((1, 768, ssl_len)))
        }
    }
}
